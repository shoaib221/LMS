...
function Task({ job }: { job: Job }) {
    ...
    return (
        <>
            <Tag />
            <div key={job.id} className="rounded-lg p-2 flex flex-row justify-between gap-4 box-13" onClick={() => router.push(`/job/${job.id}`)} >

                <div className="grid grid-cols-[7rem_1fr] gap-2" >
                    <div className="font-bold text-(--color3) " >Title: </div>
                    <div >{job.title}</div>
                    <div>Salary: </div>
                    <div >{job.salary} coins per task</div>
                    <div>Required: </div>
                    <div >{job.required_employees} employees</div>
