                ...
                {withdrawalRequests.length > 0 && withdrawalRequests.map((req) => (
                    <div key={req.id} className="box-15 rounded-lg p-2 flex justify-between flex-col lg:flex-row" >
                        <div className="grid grid-cols-[10rem_1fr] gap-2" >
                            <p className="header-3" >Withdrawer:  </p>
                            <div className="header-3" > {req.user?.name} </div>

                            <div>Contact: </div>
                            <div> {req.user?.email} </div>

                            <div>Requested Amount </div>
                            <div> {req.amount} Coins </div>
                            
                            
                            <div> Available Amount:  </div>
                            <div> {req.user.coins} Coins </div>
                        </div>

                        <br />

                        <div className="flex flex-row lg:flex-col gap-2" >
                            <button className="button-1 font-bold" onClick={() => ApproveWithdrawal('accept', req.id)} style={{ color: 'var(--color5)' }} >Accept</button>
