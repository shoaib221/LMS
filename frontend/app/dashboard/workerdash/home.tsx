            ...
            <div className="flex flex-col gap-4 p-4" >
                {
                    approvedSubmissions.length > 0 && approvedSubmissions.map((submission) => (
                        <div key={submission.id} className="box-15 grid grid-cols-[10rem_1fr]" >
                            <div className="header-3" > Task :</div>
                            <div className="header-3" >
                                {submission.job.title}
                            </div>
                            
                            <div> Earned :  </div>
                            <div> {submission.job.salary} coins </div>

                            <div> Submitted at : </div>
                            <DateDisplay date={submission.createdAt} />
                        </div>
                    )) 
                }
            </div>
